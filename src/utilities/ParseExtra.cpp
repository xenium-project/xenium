            ...
            if (c == Constants::TX_EXTRA_NONCE_IDENTIFIER && elementsRemaining > 1 && !seenNonce)
            {
                ...
                if (elementsRemaining > readNonceSize + nonceSize)
                {
                    ...
                    for (auto is = nonceData.begin(); is != nonceData.end(); is++)
                    {
                        ...
                        /* If we encounter a Payment ID field and there are enough bytes remaining in the nonce data
                           and we have not yet encountered a Payment ID, then read it out */
                        if (s == Constants::TX_EXTRA_PAYMENT_ID_IDENTIFIER && nElementsRemaining > 32 && !seenPaymentID)
                        {
                            ...
                            std::copy(is + 1, is + 33, std::begin(paymentIDHash.data));
                        ...
                        /* If we encounter an arbitrary data field and there is atleast one byte remaining
                           and we have not yet encountered the arbitrary data, then read it out */
