    ...
    */
        ...
        const double feePerByte)
    {
        const uint64_t numChunks = static_cast<uint64_t>(std::ceil(
            transactionSize / static_cast<double>(CryptoNote::parameters::FEE_PER_BYTE_V1_CHUNK_SIZE)
        ));

        return static_cast<uint64_t>(
            numChunks * feePerByte * CryptoNote::parameters::FEE_PER_BYTE_V1_CHUNK_SIZE
        );
    }
